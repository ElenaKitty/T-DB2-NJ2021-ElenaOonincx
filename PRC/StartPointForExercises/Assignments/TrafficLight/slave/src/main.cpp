...
//
// watcht it!!
// if you want to communicate to the other arduino youe need to change the
...
// buff = message when a correct message has been received (without the start
// and end character) return value : 1 : message availabe in buf 0: no message
// available -1 : an error has occured

//
// this function uses a String for the incoming message
// part of the assignment is changing this String (which is essentially a C++
// Class) to a characterarry also you need to make this code robust against
// buffer overflows.
//
...
void loop()
{
    ...
    if (messageAvailable == 1)
    {
        ...
        {
            sendReply("NACK");// we always send an ACK when a message was
                              // received.
        }
    }
